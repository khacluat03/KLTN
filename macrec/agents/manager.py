...
class Manager(Agent):
    ...
    def __init__(self, thought_config_path: str, action_config_path: str, *args, **kwargs) -> None:
        ...
        # Choose appropriate token encoders for context limit checks.
        if isinstance(self.thought_llm, AnyOpenAILLM):
            self.thought_enc = tiktoken.encoding_for_model(self.thought_llm.model_name)
        elif isinstance(self.thought_llm, AnyGeminiLLM):
            # For Gemini, avoid Hugging Face tokenizer; use a simple whitespace tokenizer.
            class _SimpleTokenizer:
                def encode(self, text: str):
                    return text.split()
            self.thought_enc = _SimpleTokenizer()
        else:
        ...
        elif isinstance(self.action_llm, AnyGeminiLLM):
            class _SimpleTokenizer:
                def encode(self, text: str):
                    return text.split()
            self.action_enc = _SimpleTokenizer()
        else:
